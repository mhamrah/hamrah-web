...
export const onPost: RequestHandler = async (event) => {
  try {
    ...
    const { response, challengeId, email, name }: CompleteRegistrationRequest = body as CompleteRegistrationRequest;
    ...
    if (!existingUser && verification.user) {
      ...
      const session = await createSession(event, sessionToken, verification.user.id);
