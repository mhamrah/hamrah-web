...
const deleteCredential = server$(async function(this: any, credentialId: string) {
  try {
    const { getCurrentUser } = await import('~/lib/auth/utils');
    ...
    const success = await deleteWebAuthnCredential(this as any, credentialId, user.id);
    if (!success) {
...
const updateCredentialName = server$(async function(this: any, credentialId: string, name: string) {
  try {
    const { getCurrentUser } = await import('~/lib/auth/utils');
    const { updateWebAuthnCredentialName } = await import('~/lib/auth/webauthn');
    ...
    const success = await updateWebAuthnCredentialName(this as any, credentialId, user.id, name);
    if (!success) {
  ...
  return (
    <div class="space-y-4">
      <div class="flex items-center justify-between">
        ...
        <span class="text-sm text-gray-500">{credentials.value.length} passkey(s)</span>
      </div>
      ...
      {credentials.value.length === 0 ? (
        <div class="text-center py-8 text-gray-500">
          ...
          <p class="text-sm">Add a passkey to enable secure, passwordless authentication</p>
        </div>
      ) : (
        <div class="space-y-3">
          {credentials.value.map((credential) => (
            <div key={credential.id} class="border border-gray-200 rounded-lg p-4 bg-white">
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  {editingId.value === credential.id ? (
                    ...
                    <div class="flex items-center space-x-2 mb-2">
                      <h4 class="font-medium text-gray-900">{credential.name || `Passkey ${credential.id.slice(-8)}`}</h4>
                      <button
                    ...
                    <div class="flex items-center space-x-4">
                      {credential.credentialDeviceType && (
                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                          {credential.credentialDeviceType === 'singleDevice' ? 'Device-bound' : 'Synced'}
                        </span>
                      )}
