...
const beginWebAuthnAuthentication = server$(async function (
  this: any,
  email?: string,
): Promise<WebAuthnBeginResponse> {
  const response = await fetch(
    `${this.url.origin}/api/webauthn/authenticate/begin`,
    {
      method: "POST",
      headers: {
...
const completeWebAuthnAuthentication = server$(async function (
  this: any,
  authResponse: any,
  challengeId: string,
): Promise<WebAuthnCompleteResponse> {
  const response = await fetch(
    `${this.url.origin}/api/webauthn/authenticate/complete`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to complete authentication: ${response.statusText}`,
    );
  }
...
const beginWebAuthnRegistration = server$(async function (
  this: any,
  email: string,
  name: string,
): Promise<WebAuthnBeginResponse> {
  const response = await fetch(
    `${this.url.origin}/api/webauthn/register/begin`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
const completeWebAuthnRegistration = server$(async function (
  this: any,
  regResponse: any,
  challengeId: string,
  email: string,
  ...
  const response = await fetch(
    `${this.url.origin}/api/webauthn/register/complete`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export const WebAuthnLogin = component$<WebAuthnLoginProps>(
  ({ onSuccess, onError }) => {
    ...
    const handleWebAuthnAuth = $(async () => {
      try {
        ...
        const beginResponse = await beginWebAuthnAuthentication(
          email.value || undefined,
        );

        if (!beginResponse.success) {
          throw new Error(
            beginResponse.error || "Failed to begin authentication",
          );
        }
    ...
    const handleWebAuthnSignUp = $(async () => {
      try {
        ...
        const beginResponse = await beginWebAuthnRegistration(
          email.value,
          name.value,
        );
        ...
        if (!beginResponse.success) {
          throw new Error(
            beginResponse.error || "Failed to begin registration",
          );
        }
            ...
            {!isSignUp.value && (
              <div class="text-center">
                ...
                >
                  {email.value
                    ? "Clear email"
                    : "Enter email for specific account"}
                </button>
              </div>
            )}
          </div>
