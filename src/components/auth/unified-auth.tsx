...
const passkeyAuthServer = server$(async function(this: any, email?: string) {
  ...
  try {
    ...
    try {
      const authOptions = await generateWebAuthnAuthenticationOptions(this as any, email);
      return { 
        success: true, 
        type: 'authentication',
        options: authOptions,
      ...
      console.log('Authentication option generation failed, trying registration:', authError);
...
const completePasskeyAuth = server$(async function(
  ...
  const { generateSessionToken, createSession, setSessionTokenCookie } = await import('~/lib/auth/session');
  ...
  try {
    if (type === 'authentication') {
      ...
      const result = await verifyWebAuthnAuthentication(this as any, response, challengeId);
      ...
      return { 
        ...
        needsProfile: !email || email.includes('@temp.com') || !name || name === 'New User'
      };
    }
  } catch (error: any) {
...
export const UnifiedAuth = component$<UnifiedAuthProps>((props) => {
  ...
  const handlePasskeyAuth = $(async () => {
    ...
    try {
      ...
      const beginResult = (await (passkeyAuthServer as any)(showEmailInput.value ? email.value : undefined)) as any;

      if (!beginResult.success) {
        throw new Error(beginResult.error || 'Failed to begin passkey authentication');
      }
      ...
      if (!completeResult.success) {
        throw new Error(completeResult.error || 'Failed to complete passkey authentication');
      }
  ...
  return (
    <div class="space-y-6">
      <div class="text-center">
        ...
        <p class="mt-2 text-gray-600">Sign in to your account or create a new one</p>
      </div>
      ...
      <div class="space-y-4">
        ...
        <div class="space-y-3">
          {showEmailInput.value && (
            <div>
              ...
              <p class="mt-1 text-xs text-gray-500">
                Help us find your existing account or leave blank for anonymous signin
              </p>
            </div>
          )}
          ...
          <div class="text-center">
            ...
            >
              {showEmailInput.value ? 'Hide email field' : 'Add email for account linking'}
            </button>
          </div>

          <div class="text-center text-xs text-gray-500">
            Use your device's built-in security (Face ID, Touch ID, Windows Hello, or security key)
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3">
          <button
            ...
            disabled={isLoading.value}
            class="w-full inline-flex justify-center items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
          >
